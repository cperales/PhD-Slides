...
Conjunto de datos de entrenamiento $\mathcal{D} = \{  ( \mathbf{x}_1, y_1), \ldots (\mathbf{x}_N, y_N) \} = \{ (\mathbf{x}_n, y_n) \}_{n=1}^N$. Buscamos un predictor $f : \mathcal{X} \rightarrow \mathcal{Y}$,
...
\frame{
En general, no podemos conocer exactamente la relación entre el dominio de las features, $\mathcal{X}$, y el dominio target, $\mathcal{Y}$. Pero podemos interpolar esos dominios conociendo $N$ patrones, % con features $\mathbf{x}_n \in \mathcal{X}$, y target $\mathbf{y}_n \in \mathcal{Y}$.


\begin{equation}
\label{eq:f}
f (\mathbf{x}_n; \theta) \approx y_n, \: \forall n .
\end{equation}
...
\min_{\theta} \text{ Error} ( \left( f(\mathbf{x}_1; \theta ), y_1 \right), \ldots, \left( f(\mathbf{x}_N; \theta ), y_N \right) )
\end{equation}
}

%\frame{
%En problemas de regresión, clásicamente se utiliza el error cuadrático medio, o \textit{Mean Squared Error}, que es equivalente a calcular la norma $L2$ entre el target $y_n, \: n=1, \ldots, N$ y la predicción $f (\mathbf{x}_n), \:n=1, \ldots, N$.
%
%\begin{equation}
%\label{eq:mse}
%\min_{\theta} \frac{1}{N} \sum_{n=1}^{N} { \left( f(\mathbf{x}_n; \theta ) - y_1 \right) }^2
%\end{equation}
%
%
%}
...
\subsection{Extreme Learning Machine}  % 5 minutos
...
El predictor es de la siguiente forma:
	...
	\begin{equation}
		\label{eq:elm}
		f(\mathbf{x}) = h(\mathbf{x})' \mathbf{\beta}
	\end{equation}
	
Donde:
	
\begin{itemize}
	\item $\mathbf{x} \in \mathbb{R}^K$.
	\item $y \in \mathbb{R}$.
	\item $h: \mathbb{R}^K \rightarrow \mathbb{R}^D$, ${h}(\mathbf{x}) = (\phi(\mathbf{x};\mathbf{w}_{d},b_{d}),d = 1,\ldots,D)$,
	\item $\mathbf{\beta} \in \mathbb{R}^{D \times 1}$.
\end{itemize}
...
\begin{equation}
	\min_{\mathbf{\beta}} {\lVert \mathbf{H} \mathbf{\beta} - \mathbf{Y} \rVert}_2^2 + C {\lVert \mathbf{\beta} \rVert}_2^2.
\end{equation}
...
\begin{itemize}
	...
	\item $\mathbf{H} = \left(\mathbf{h}' \left(\mathbf{x}_{1}\right), \ldots, \mathbf{h}' \left(\mathbf{x}_{N}\right)\right)'$.
...
\mathbf{\beta} = \left( \mathbf{H}' \mathbf{H} + C \mathbf{I} \right)^{-1} \mathbf{H}' \mathbf{Y}
\end{equation}
}
...
\frame[allowframebreaks]{\frametitle{RE-ELM}
	...
	\begin{equation}
	\label{eq:cosine}
	 \cos\left(\angle\left(\mathbf{u},\mathbf{v}\right)\right) = \frac{\left\langle \mathbf{u},\mathbf{v}\right\rangle }{\left\Vert \mathbf{u}\right\Vert \left\Vert \mathbf{v}\right\Vert } \in [-1, 1],
	\end{equation}
	
	El cuadrado del producto escalar de dos vectores será $0$ si los vectores son perpendiculares, $\mathbf{u} \perp \mathbf{v}$, y la norma de estos al cuadrado si los vectores son paralelos, $\mathbf{u} \parallel \mathbf{v}$.
	
	\begin{equation}
	\label{eq:diversity_vector}
	0 \le \left\langle \mathbf{u},\mathbf{v}\right\rangle ^{2} \le \left\Vert \mathbf{u}\right\Vert ^{2}\left\Vert \mathbf{v}\right\Vert ^{2}.
	\end{equation}
	...
	\min_{\mathbf{\beta}^{(s)}\in \mathbb{R}^{D \times 1}}  \left( {\lVert \mathbf{\beta}^{(s)} \rVert}^2 + C {\lVert\mathbf{H}\mathbf{\beta}^{(s)}-\mathbf{Y}\rVert}^2 + N \sum_{k=1}^{s-1} \alpha^{(k)}  \left\langle\mathbf{\beta}^{(s)} , \mathbf{u}^{(k)}\right\rangle^2 \right).
	\end{equation}

	Con $\mathbf{u}^{(k)} = \frac{\mathbf{\beta}^{(k)}}{\lVert \mathbf{\beta}^{(k)} \rVert}$. Se proponen los parámetros $\alpha^{(s)}, \: s=1,\ldots,S$, tanto como pesos de la diversidad en la minimización como pesos en la predicción final,
	
	
	\begin{equation}
	f(\mathbf{x}) = \sum_{s=1}^{S} \alpha^{(s)} \left(\mathbf{h}'\left(\mathbf{x}\right)\mathbf{\beta}^{(s)}\right).
	\end{equation}	
...
%
%\begin{equation}
%f(\mathbf{x}) = \sum_{s=1}^{S} \alpha_{s}\left(\mathbf{h}'\left(\mathbf{x}\right)\mathbf{\beta}^{(s)}\right).
%\end{equation}
...
\frame[allowframebreaks]{\frametitle{NC-ELM}
	...
	Diversidad de la predicción de cada elemento del ensemble sobre conjunto de entrenamiento con respecto al total, $\mathbf{F}$. 	
	La solución se construye $r = 1, \ldots, R$ iteraciones,
	
	\begin{equation}
	\mathbf{f}_{(r)}( \mathbf{x} )=\frac 1 S \sum_{s=1}^S \mathbf{h}^{(s)'}(\mathbf{x}) {\mathbf{\beta}}_{(r)}^{(s)},
	\end{equation} 
	
	En la primera iteración, son ELM con funciones de mapeo distintas (diversidad kernel):
	
		\begin{equation}
	\label{eq:Diverse_matrix_ELM_min}
	\min_{\mathbf{\beta}^{(s)}_{(1)} \in \mathbb{R}^{D \times 1}}  C {\lVert \mathbf{\beta}^{(s)}_{(1)} \rVert}^2 + {\lVert\mathbf{H}^{(s)} \mathbf{\beta}^{(s)}_{(1)}-\mathbf{Y}\rVert}^2.
	\end{equation}
	
%	\begin{equation}
%		\boldsymbol{F}_{(1)} = \left( \frac{1}{S} \sum_{s=1}^S f^{(s)}_{(1)} (\boldsymbol{x}_{1}), \ldots, \frac{1}{S} \sum_{s=1}^S f^{(s)}_{(1)} (\boldsymbol{x}_N) \right)' 
%	\end{equation}
	
	Con $ \mathbf{F}_{(r-1)}$ como la suma de las predicciones del ensemble sobre el set de entrenamiento. 
	 En las siguientes iteraciones,
	
	\begin{equation}
	\label{eq:bj}
	\min_{\mathbf{\beta}^{(s)}_{(r)}}  \Vert\mathbf{H}^{(s)}\mathbf{\beta}^{(s)}_{(r)} - \mathbf{Y} \Vert^2 + C \Vert \mathbf{\beta}^{(s)}_{(r)} \Vert^2 + \lambda\left \langle  \mathbf{H}^{(s)}\mathbf{\beta}^{(s)}_{(r)}, \mathbf{F}_{(r-1)}\right \rangle^2 .
	\end{equation}
	
	La solución es iterada, y para cada elemento,
	
	\begin{equation}
\mathbf{A}_{j, (r)}^{(s)} = \left( \frac{\mathbf{I}}{C}+\mathbf{H}^{(s)'}\mathbf{H}^{(s)}+\frac{\lambda}{C}\mathbf{H}^{(s)'}\mathbf{F}_{j, (r-1)} \mathbf{F}_{j, (r-1)}^{'} \mathbf{H}^{(s)} \right)
	\end{equation}
	
	\begin{equation}
	{\mathbf{\beta}}_{j, (r)}^{(s)} = \left(\mathbf{A}_{j, (r)}^{(s)} \right)^{-1} \mathbf{H}^{(s)'}\mathbf{Y}_j,
	\end{equation}
}
...
\frame[allowframebreaks]{\frametitle{GNC-ELM}
	...
	\begin{eqnarray}
		\Phi(f |C, \lambda) = \frac{1}{S} \sum_{s=1}^S \phi(f^{(s)}|C,\lambda) \nonumber\\
		= \frac{1}{S} \sum_{s=1}^S \text{reg}(f^{(s)})+ C\text{fit}\left( \frac{1}{S} \sum_{s=1}^S f^{(s)}\right) \nonumber\\
		+\lambda \frac{1}{S} \sum_{s=1}^S \text{div}(f^{(s)}|f_1,\ldots,f^{(S)})
	\end{eqnarray}
	
	\begin{equation}
	\left( \frac{\mathbf{I}}{\gamma}+\mathbf{H}_k'\mathbf{H}_k \right) \mathbf{\beta}_k + (1 + \delta) \sum_{s\neq k} (\mathbf{H}_k'\mathbf{H}_s) \mathbf{\beta}_s = (S + \delta (S - 1))\mathbf{H}_k'\mathbf{y},
	\end{equation}
	
	\begin{equation}\arraycolsep=1.0pt
	\mathbf{A} =  \begin{pmatrix}
	& \frac{\mathbf{I}}{\gamma}+ \mathbf{H}_1'\mathbf{H}_1 &\ldots& (1 + \delta) \mathbf{H}_1'\mathbf{H}_s  & \ldots & (1 + \delta) \mathbf{H}_1'\mathbf{H}_S  \\
	& \vdots & \ddots & \vdots & \vdots & \vdots \\
	& (1 + \delta) \mathbf{H}_s'\mathbf{H}_1 & \ldots & \frac{\mathbf{I}}{\gamma} + \mathbf{H}_s'\mathbf{H}_s & \ldots & (1 + \delta) \mathbf{H}_s'\mathbf{H}_S\\
	& \vdots & \vdots & \vdots & \ddots & \vdots \\
	& (1 + \delta) \mathbf{H}_S'\mathbf{H}_1 & \ldots & (1 + \delta) \mathbf{H}_S'\mathbf{H}_s & \ldots & \frac{\mathbf{I}}{\gamma}+ \mathbf{H}_S'\mathbf{H}_S
	\end{pmatrix}
	\end{equation}
	
	\begin{equation}
	\mathbf{\beta}=\mathbf{A}^{-1}(S + \delta (S - 1) ) \mathbf{H}'\mathbf{y},
	\end{equation}
	
}

\frame{\frametitle{Resultados I}
...
\frame{\frametitle{Resultados II}
...
\frame[allowframebreaks]{\frametitle{Ventajas/desventajas}
	...
	\begin{enumerate}
		\item Inversión matricial única, resultado exacto. Todos los elementos del ensemble se resuelven de una vez, y aportan lo mismo.
		\item Los hiperparámetros intervienen en la resolución matricial.
		\item Reducción del error de conjunto, no individual.
	\end{enumerate}
